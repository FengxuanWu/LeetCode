...
int longestDecomposition(string text) 
{
	...
	vector<int> dp(text.size() / 2 + 1, 1);
	int check_point = text.size() / 2 + 1;
	int start_point = text.size() / 2 - 1;
	dp.back() = 1;
	while (start_point >= 0)
	{
		if (text[start_point] == text[2 * (text.size() / 2) - start_point])
			dp[start_point] = dp[start_point + 1] + 2;
		else
		{
			int check_start = start_point;
			int check_end = check_point;
			while (check_end < 2 * (text.size() / 2) - check_start && text[check_end] != text[check_start])
				check_end++;
			
			while (text[check_start] == text[check_end])
				check_start++, check_end++;

			if (check_end == 2 * (text.size() / 2) - check_start + 1)
			{
				dp[start_point] = dp[check_start] + 2;
				check_point = check_end;
			}						
		}
		cout << dp << endl;
		start_point--;
