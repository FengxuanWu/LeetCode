...
//bool detect_ring(vector<vector<int>>& graph, int curr, map<int, int> visited)
//{
...
//			visited[curr] = true;
//			if (i >= curr)
//				res = detect_ring(graph, i, visited);
//				if (res)
//					return res;
//		}
//	}
...
{	
	//string s = "3[a2[c]]";
	vector<int> nums = { 1, 3, 4, 5, 7, 8, 2 };
	vector<int> nums_cpy = nums;
	for (int i = 0; i < 10000; i++)
	{
		/*cout << "origin nums: " << nums << endl;
		cout << "origin nums_cpy: " << nums_cpy << endl;*/
		nextPermutation(nums);
		next_permutation(nums_cpy.begin(), nums_cpy.end());
		if (nums != nums_cpy)	
		{
			cout << nums << endl << nums_cpy << endl;
		}	
	}
	return 0;
}
